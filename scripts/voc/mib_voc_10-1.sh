...
MODEL0=checkpoints/step/att_top_25_voc/10-1-voc_MiB_0.pth
...
CUDA_VISIBLE_DEVICES=0 python3 -m torch.distributed.launch --master_port ${PORT} --nproc_per_node=1 attribute.py --date ${START_DATE} --data_root ${DATA_ROOT} --overlap --batch_size ${BATCH_SIZE} --dataset ${DATASET} --name ${NAME} --task ${TASK} --step 7 --lr 0.001 --method ${METHOD} --opt_level O2 ${OPTIONS}
...
CUDA_VISIBLE_DEVICES=0 python3 -m torch.distributed.launch --master_port ${PORT} --nproc_per_node=1 attribute.py --date ${START_DATE} --data_root ${DATA_ROOT} --overlap --batch_size ${BATCH_SIZE} --dataset ${DATASET} --name ${NAME} --task ${TASK} --step 10 --lr 0.001 --method ${METHOD} --opt_level O2 ${OPTIONS}
